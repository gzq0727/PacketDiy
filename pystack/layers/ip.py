...
class IPProtocol(Layer):
    ...
    def send_packet(self, packet, **kwargs):
        ...
        if kwargs.has_key("MPLS"):
            self.transfer_packet(self.forge_packet(packet,**kwargs["IP"]),"MPLS",**kwargs)
        else:
            ...
            if len(p) > self.MTU:
                ...
                for frag in fragments:
                    self.transfer_packet(frag, "default", **kwargs)
            else:
                self.transfer_packet(p, "default", **kwargs)
        '''
        ipordn = kwargs["dst"]
        if re.match(self.ipregex, ipordn):
         ...
         '''
    def transfer_packet(self, packet, low_layer_name, **kwargs):
        """
        ...
        """
        self.lowerLayers[low_layer_name].send_packet(packet, **kwargs)
